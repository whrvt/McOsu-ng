...
{
	// NOTE: windows 10 gets wndproc congestion if the message pump is very slow (< ~36 fps), causing all raw WM_INPUT + keyboard events to become massively delayed (as bad as mouse_fakelag)
	// for some reason this is caused by having a realtimestylus + event handler registered, so the workaround is to only register it if the user explicitly enabled win_realtimestylus
	...
	if (win_realtimestylus->getBool())
		return InitRealtimeStylusEx(hInstance, hWnd);

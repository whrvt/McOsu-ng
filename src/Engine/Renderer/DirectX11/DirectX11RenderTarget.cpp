...
void DirectX11RenderTarget::init()
{
	...
	hr = graphics->getDevice()->CreateDepthStencilView(m_depthStencilTexture, &depthStencilViewDesc, &m_depthStencilView);
	if (FAILED(hr))
	{
	...
	hr = graphics->getDevice()->CreateShaderResourceView(m_renderTexture, &shaderResourceViewDesc, &m_shaderResourceView);
	if (FAILED(hr))
	{
...
void DirectX11RenderTarget::disable()
{
	...
	// HACKHACK: slow af
	{
		static_cast<DirectX11Interface*>(g.get())->getDeviceContext()->OMSetRenderTargets(1, &m_prevRenderTargetView, m_prevDepthStencilView);

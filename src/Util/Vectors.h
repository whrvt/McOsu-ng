...
struct Vector2 : public glm::vec2
{
	...
	Vector2 &normalize()
	{
		if (glm::length(static_cast<const glm::vec2 &>(*this)) < VECTOR_FLOAT_NORMALIZE_EPSILON)
			return *this;
...
struct Vector2d : public glm::dvec2
{
	...
	Vector2d(const glm::dvec2 &other)
	    : glm::dvec2(other)
	{
	}
	...
	[[nodiscard]] double distance(const Vector2d &vec) const { return glm::distance(static_cast<const glm::dvec2 &>(*this), static_cast<const glm::dvec2 &>(vec)); }
	Vector2d &normalize()
	{
		if (glm::length(static_cast<const glm::dvec2 &>(*this)) < VECTOR_DOUBLE_NORMALIZE_EPSILON)
			return *this;
		*this = glm::normalize(static_cast<const glm::dvec2 &>(*this));
		return *this;
	}
	[[nodiscard]] double dot(const Vector2d &vec) const { return glm::dot(static_cast<const glm::dvec2 &>(*this), static_cast<const glm::dvec2 &>(vec)); }
	[[nodiscard]] bool equal(const Vector2d &vec, double e) const
	{
		return glm::all(glm::lessThan(glm::abs(static_cast<const glm::dvec2 &>(*this) - static_cast<const glm::dvec2 &>(vec)), glm::dvec2(e)));
	}
...
struct Vector4 : public glm::vec4
{
	...
	Vector4 &normalize()
	{
		if (glm::length(static_cast<const glm::vec4 &>(*this)) < VECTOR_FLOAT_NORMALIZE_EPSILON)
			return *this;
